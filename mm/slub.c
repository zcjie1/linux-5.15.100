...
{
	unsigned long freepointer_addr; // freepointer 在 object 内存区域的起始地址
	void *p; // 指向下一个空闲对象的 free_pontier
...
static struct page *allocate_slab(struct kmem_cache *s, gfp_t flags, int node)
{
	...
	if (page_is_pfmemalloc(page))
		SetPageSlabPfmemalloc(page); // 设置PG_active标志位
	 ...
	 * shuffle = true 表示随机初始化 freelist
	 * shuffle = false 表示按照正常的顺序初始化 freelist
	*/
	...
	if (!shuffle) {
		...
		// 用 slub 中的第一个空闲对象作为 freelist 的头结点
		page->freelist = start;
...
{
	// 若置位了GFP_SLAB_BUG_MASK，则不从DMA32和HIGHMEM区域分配内存
...
/* 从node partial列表获取slab，并将他从列表中移除
 * 
			      ...
			      struct page **ret_page, gfp_t gfpflags)
{
	...
	// 指向从 partial 列表 slub 中申请到的对象
	void *object = NULL;

	// 记录 slab cache 本地 cpu 缓存 kmem_cache_cpu 中所缓存的空闲对象总数
	unsigned int available = 0;

	unsigned long flags;

	// 记录遍历到的 slub 中包含的剩余空闲对象个数
	int objects;
	...
	// 遍历 kmem_cache_node 的 partial 列表
	list_for_each_entry_safe(page, page2, &n->partial, slab_list) {
		...
		// 从partial列表获取slab，并将他从列表中移除
		// object为当前slub剩余对象数, page为当前遍历到的slub
		t = acquire_slab(s, n, page, object == NULL, &objects);

		// 若t为NULL，说明node partial也为空
		if (!t)
			break;
		...
		if (!object) {
			// 第一次循环，将第一个 slub 缓存进 kmem_cache_cpu 中
			*ret_page = page;
			stat(s, ALLOC_FROM_PARTIAL);
			object = t;
		} else {
			// 第二次及之后的循环，将slub 填充进kmem_cache_cpu的partial链表
			put_cpu_partial(s, page, 0);
			stat(s, CPU_PARTIAL_NODE);
		}

		// 若 slab cache没有cpu_partial 或者 空闲对象数大于cpu_partial的一半
			 ...
			 struct page **ret_page)
{
	...
	// 按照访问距离，遍历 searchnode 之后的 numa node，进行跨节点内存分配
 ...
 * by the caller.
 * 
 * 将cpu->freelist指向的object重新加入到page->freelist的怀抱中
 * 取消cpu缓存slab的frozen标志位，并将该slab放置到node的列表上
 * 过载了就把slab直接free掉
 * 
 */
			    ...
			    void *freelist)
{
	...
	// 若page->freelist不为空，说明其他CPU曾经释放对象到当前slab中，将其放至表尾
	if (page->freelist) {
	 ...
	 * 
	 * 获取cpu->freelist指向的空闲对象数 = free_delta
	 */
	freelist_tail = NULL;
...
redo:
	...
	old.freelist = READ_ONCE(page->freelist); // 其他CPU释放的空闲对象
	old.counters = READ_ONCE(page->counters);
	VM_BUG_ON(!old.frozen);

	/* Determine target state of the slab */
	new.counters = old.counters;
	if (freelist_tail) { // 若slab未被全部分配
		// counters与inuse为联合体
		new.inuse -= free_delta;

		// 将其他CPU释放的空闲对象接入freelist_tail
		set_freepointer(s, freelist_tail, old.freelist);

		// 更新new page的freelist
		new.freelist = freelist;
	} else
		// 若slab全被分配，则将其他CPU释放的空闲对象列表赋给new.freelist
		new.freelist = old.freelist;

	new.frozen = 0;

	// 若未有对象被分配 且 node partial列表过载，直接free
	if (!new.inuse && n->nr_partial >= s->min_partial)
		m = M_FREE;
	else if (new.freelist) { // 若freelist不为空，转移至node partial列表
		m = M_PARTIAL;
		if (!lock) {
			lock = 1;
			/*
...
/* 返回page->freelist，可能有其他CPU释放对象到此slab中
 * 因为无法访问kmem_cache_cpu的指针，只能增加page->freelist
...
static inline void *get_freelist(struct kmem_cache *s, struct page *page)
{
	...
	do {
		...
		freelist, counters,
		NULL, new.counters,
		"get_freelist")); // 获取page->freelist和page->counters
...
redo:
	...
	local_lock_irqsave(&s->cpu_slab->lock, flags); // 获取kmem_cache_cpu的锁
	...
	// 若第一个空闲对象不为空，则从 cpu 本地缓存中的 slub 中直接分配对象
	if (freelist)
		goto load_freelist;

	/** 到达这里，说明上一步得到的freelist为空 
	 * 可能有其他CPU释放对象到此slab中，并更新了page->freelist
	 * 获取page->freelist作为新的freelist
	*/
	freelist = get_freelist(s, page);

	// 若page->freelist也为空，说明slab已满
	if (!freelist) {
		c->page = NULL;
		c->tid = next_tid(c->tid);
		local_unlock_irqrestore(&s->cpu_slab->lock, flags);
...
deactivate_slab:  // 进入慢速路径——kmem_cache_cpu->page上没有空闲对象
	...
	deactivate_slab(s, page, freelist); // 转移slab

new_slab:

	// 若kmem_cache_cpu的partial列表非空
	if (slub_percpu_partial(c)) {
		...
		// 获取partial的第一个slab，并加入到kmem_cache_cpu缓存
		page = c->page = slub_percpu_partial(c);

		// 将page->next设置为新的partial列表头
...
new_objects: // kmem_cache_cpu的partial列表中也没有slab
	 ...
	 */
	freelist = page->freelist; // 获取第一个空闲对象的指针
	page->freelist = NULL;
...
retry_load_page:
	...
	// 重新填充kmem_cache_cpu->page链表
	c->page = page;

	goto load_freelist;

return_single:
	// 仅返回一个空闲对象指针，将slab其他的空闲对象加入到node partial列表(或free)
...
redo:
	 ...
	 * 
	 * 获取一个kmem_cache_cpu后，进程可能在不同CPU上运行
	 * 但要保证执行cmpxchg时，当前CPU与初始CPU相同
	 *
	 ...
	 * 
	 * 在一个kmem_cache_cpu上，tid对于per cpu和per operation都是独一无二的
	 * 确保对一个kmem_cache_cpu的operation是原子的
	 */

	// 本地 cpu 缓存的 slub 中第一个空闲对象
	object = c->freelist;
	 ...
	 * 
	 * 使能了硬实时抢占时(将普通spinlock替换为可睡眠的mutex, 即进程持有锁时可以睡眠)，
	 * 若进程A在slow_path中持有了kmem_cache_cpu->lock发生调度
	 * 进程B抢占，并进入了lockless fastpath，此时锁就失效了
	 * 为避免这种情况，使能硬实时抢占时，所有进程都通过慢分配路径
	 * 
	 * 若 "使能硬实时抢占(部分中断可抢占)" || "无空闲对象" || "无缓存slub" || "缓存slub非本地页"
	 * 直接进入慢速分配路径
	 */
	if (IS_ENABLED(CONFIG_PREEMPT_RT) ||
	    unlikely(!object || !page || !node_match(page, node))) {
		object = __slab_alloc(s, gfpflags, node, addr, c); // 进入慢分配路径
	} else {
		// 快速分配路径
		 ...
		 * 
		 * 比较freelist和tid是否发生变化
		 * 若发生变化，则说明其他进程抢占了CPU并获取了空闲对象，需要重试
		 * 若未发生变化，则更新当前freelist
		 */
...
/* 返回符合size的slab_order，且保证较小的内存碎片
		...
		unsigned int fract_leftover)
{
	...
	/**
	 * 若 2^min_order个内存页可以存放的对象个数超过 32767 限制
	 * 返回 size * MAX_OBJS_PER_PAGE 所需要的 order 减 1
	*/
	if (order_objects(min_order, size) > MAX_OBJS_PER_PAGE)
		return get_order(size * MAX_OBJS_PER_PAGE) - 1;

	/**
	 * 从 slab 所需要的最小 order 到最大 order 之间开始遍历
	 * 查找能够使 slab 碎片最小的 order 值
	*/
...
static inline int calculate_order(unsigned int size)
{
	...
	while (min_objects > 1) {
		...
		fraction = 16;
		while (fraction >= 4) {
			// 根据当前 fraction 计算 order，查找出能够使 slab 产生碎片最小化的 order 值
			order = slab_order(size, min_objects,
					slub_max_order, fraction);
			
			// order 不能超过 max_order，否则需要降低 fraction，放宽对碎片的要求限制
...
static inline int alloc_kmem_cache_cpus(struct kmem_cache *s)
{
	 ...
	 * 
	 * __alloc_percpu 函数用于分配 percpu 类型的结构体
	 */
...
{
	// slab 所属的 page (或复合页首页)
	struct page *page;
	struct kmem_cache_node *n;

	BUG_ON(kmem_cache_node->size < sizeof(struct kmem_cache_node));

	// 根据 kmem_cache 结构中的 oo 属性向指定 node 节点所属的伙伴系统申请 2^order 个内存页 page
	// 这2^order个内存页就对应一个slab，后续挂载在kmem_cache_node的partial列表上
	...
	BUG_ON(!n);
#ifdef CONFIG_SLUB_DEBUG
	/* 根据 slab cache 中的 flag 初始化 kmem_cache_node 对象(red_zone, posion等) */
	init_object(kmem_cache_node, n, SLUB_RED_ACTIVE);
	init_tracking(kmem_cache_node, n);
#endif
	n = kasan_slab_alloc(kmem_cache_node, n, GFP_KERNEL, false);

	// 获取对象 n 中的 free_pointer 指针,指向 n 的下一个空闲对象
	page->freelist = get_freepointer(kmem_cache_node, n);

	// 表示 slab 中已经有一个对象被使用了
	page->inuse = 1;

	// 当前slab尚未被加入到cpu缓存列表中
	page->frozen = 0;
	 ...
	 * 
	 * 将新创建出来的 slab （page表示），添加到对象n kmem_cache_node结构）的 partial 列表头部
	 */
...
static int init_kmem_cache_nodes(struct kmem_cache *s)
{
	...
	for_each_node_mask(node, slab_nodes) {
		...
		if (slab_state == DOWN) {
			/**
			 * 如果此时 slab allocator 体系还未建立，则调用该方法分配 kmem_cache_node 结构(对象池)，并初始化
			 * slab cache 的正常创建流程不会走到这个分支
			 * 该分支用于在内核初始化的时候创建 kmem_cache_node 对象池使用
			*/
...
static void set_cpu_partial(struct kmem_cache *s)
{
	 ...
	 * 
	 * 空闲对象size越小, cpu_partial(每个CPU允许持有的空闲对象数)越高
	 */
	if (!kmem_cache_has_cpu_partial(s)) // 判断 kmem_cache_cpu 是否包含有 partial 列表
...
static int calculate_sizes(struct kmem_cache *s, int forced_order)
{
	 ...
	 * 
	 * SLAB_POISON: 将对象所占内存用 0x6b 填充，并用0xa5填充object size的最后一个字节(检测越界访问)
	 ...
	 * 
	 * SLAB_RED_ZONE：在对象前后插入 red zone 红色区域（特定字节 0xbb），防止对象溢出越界
	 * size == s->object_size 表示对象 object size 与 word size 本来就是对齐的
	...
	/**
	 * 若开启了 RCU 保护 || 设置了对象 poison || 设置了对象的构造函数
	 * 则额外增加一个 word size 大小的空间来存放 free pointer
	*/
	...
	if (flags & SLAB_STORE_USER)
		 ...
		 * 需要在对象的末尾增加两个 struct track 结构，存储分配和释放的信息
		 */
...
#ifdef CONFIG_SLUB_DEBUG
	// 在对象内存区域的struct track后增加一段word_size
	// 在对象内存区域的左侧增加一段red_zone
	...
	s->allocflags = 0;
	if (order)
		s->allocflags |= __GFP_COMP; // slab 所需要的内存页多于 1 页时，则向伙伴系统申请复合页

	if (s->flags & SLAB_CACHE_DMA)
		s->allocflags |= GFP_DMA; // 从 DMA 区域中获取适用于 DMA 的内存页

	if (s->flags & SLAB_CACHE_DMA32)
		s->allocflags |= GFP_DMA32; // 从 DMA32 区域中获取适用于 DMA 的内存页

	if (s->flags & SLAB_RECLAIM_ACCOUNT)
		s->allocflags |= __GFP_RECLAIMABLE; // 申请可回收的内存页
	...
	if (oo_objects(s->oo) > oo_objects(s->max)) // 初始时 max 和 oo 相等
		s->max = s->oo;

	// 若能容纳空闲对象，则返回1，不能则返回0
...
static int kmem_cache_open(struct kmem_cache *s, slab_flags_t flags)
{
...
#endif
	// 计算 slab 中对象的整体内存布局所需要的 size, 并初始化相关数据
	 ...
	 * 
	 * 设置min_partial，限制numa node中缓存的slab个数上限
	 */
	set_min_partial(s, ilog2(s->size) / 2);

	// 设置cpu_partial, 限制cpu本地缓存partial列表中空闲对象个数的上限
	set_cpu_partial(s);
...
static struct kmem_cache * __init bootstrap(struct kmem_cache *static_cache)
{
	...
	// 将静态的 kmem_cache 对象深拷贝到最终的 kmem_cache 对象 s 中
	...
	// 遍历 node cache 数组
	// 修正 kmem_cache_node 结构中 partial 链表中包含的 slab （ page 表示）对应 page 结构的 slab_cache
	for_each_kmem_cache_node(s, node, n) {
		struct page *p;
...
void __init kmem_cache_init(void)
{
	...
	if (__slub_debug_enabled())
		no_hash_pointers_enable(NULL); // 输出notice
...
/** slab cache创建核心函数
 * 初始化 kmem_cache 结构中的重要属性
 * 创建并初始化 kmem_cache_cpu 和 kmem_cache_node 结构
*/
int __kmem_cache_create(struct kmem_cache *s, slab_flags_t flags)
{
	 ...
	 *  
	 * 检查内核中 slab 分配器是否初始化完毕
	 * 状态为 FULL 则初始化完毕
	 * slab_sysfs_init 函数中将 slab_state 设置为 FULL
	 * 
	 * state小于等于UP时，还不能将slab cache导出到sysfs
	*/
	if (slab_state <= UP)
		return 0;

	// 在 sys 文件系统中创建 /sys/kernel/slab/name 目录
